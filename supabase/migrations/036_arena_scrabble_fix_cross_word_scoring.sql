-- =============================================================================
-- 036_arena_scrabble_fix_cross_word_scoring.sql
--
-- Bug fix: single-tile scoring double-counted a tile that extends ONLY a
-- vertical (down) word. The cross-word loop chose its walk direction from a
-- condition that could not tell whether the auto-axis main word had landed
-- horizontally or vertically, so for a one-tile play that formed only a
-- vertical word it re-walked that same word as a "cross word" and added it
-- twice.
--
-- Fix: track which axis the main word actually lies on (v_main_axis) and make
-- cross words STRICTLY perpendicular to it. Behaviour is identical to 035 for
-- every other case (multi-tile across/down plays, single tiles that extend a
-- horizontal word, single tiles at an intersection) — only the
-- vertical-only single-tile case changes, and now scores correctly.
--
-- Run order: after 035_*.sql. CREATE OR REPLACE preserves existing grants.
-- =============================================================================
...
declare
  ...
  v_main_axis        text := '';            -- which axis the main word lies on (cross-word fix)
begin
  ...
  if p_kind = 'pass' then
    ...
    for v_n in 1 .. array_length(v_placed_idx, 1) loop
      ...
      if v_main_axis = 'across' then   -- cross words are perpendicular to the main word
        v_r := v_placed_rows[v_n];
        while v_r > 0 and substr(v_new_board, (v_r - 1) * v_dim + v_c + 1, 1) <> '.' loop v_r := v_r - 1; end loop;
        v_word_chars := '';
        v_word_score := 0;
