...
| 27 | ingest-abs-population-hist | ABS Historical Population HPDC3 Table 1 (1980–2000; a static 2021-vintage file — read back from rdp_raw_series once seeded, downloaded only on a first seed) + ABS Data API ASGS2021 GCCSA codes 1GSYD..8ACTE (2001+) | `population_gccsa` (8 capitals, Greater boundaries) | A | rdp_raw_series (completeness guard: every capital, every year 1980..latest) + forge_data_status `population_gccsa` → PUBLISH step `sync-gccsa-to-population-capitals` copies it over the capitals’ `population` for the years both carry. **In the GATHER loop since 2026-09-14** (was a one-off hand run 2026-07-18 — the pipeline board flagged it >45d while the Forge grid, which had no card for it, stayed green; the Population card now folds this leg in). |
